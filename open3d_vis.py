...
from yolo_inference import detect_car
...
def viewer_3d(scans: client.Scans, paused: bool = False, pca=None, meta=None) -> None:
    ...
    image = create_canvas(int(metadata.format.columns_per_frame * 2),
                          int(metadata.format.pixels_per_column * 2))

    

    def update_data(vis: o3d.visualization.Visualizer, pca=None, meta=None):
        xyz = xyzlut(scan.field(range_for_field(fields[field_ind])))

        xyz_destaggered = client.destagger(metadata, xyzlut(scan))    
        range_field = scan.field(client.ChanField.RANGE)
        range_val = client.destagger(meta, range_field)
        ...
        # detect cars
        if pca is not None:
            gray_img, range_rois = detect_car(client, scan, pca, meta)
        gray_img = gray_img / 255.0
        height, width = gray_img.shape[0:2]
        gray_img = cv2.resize(gray_img, (int(width * 2), int(height * 2)), cv2.INTER_NEAREST)
        canvas_set_image_data(image, gray_img)
        # canvas_set_image_data(image, client.destagger(metadata, gray_img))

        # add bbox to 3d points
        # (128, 1024, 3) (131072, 3)
        cloud.points = o3d.utility.Vector3dVector(xyz.reshape((-1, 3)))

        for x1, y1, x2, y2 in range_rois:
            range_roi = range_val[y1: y2, x1: x2]

            xyz_range = xyz[y1: y2, x1: x2]
            xyz_flat = xyz_range.reshape((-1, 3))
            z_min = min(xyz_flat, key=lambda x: x[2])[2]

            range_roi[np.where(range_roi == 0)] = 9999999
            min_range = np.percentile(range_roi.reshape(-1), 8)
            for i in range(x1, x2):
                for j in range(y1, y2):
                    if range_val[j][i] < min_range * 1.08:
                        _, _, z = xyz_destaggered[j][i]
                        if z - z_min > 0.5:
                            color_img[j][i] = [0.0, 1.0, 0.0]


        # remove ground points
        # z_threshold = 0.5
        # z_min = min(cloud.points, key=lambda x: x[2])[2]


        # for x1, y1, x2, y2 in range_rois:
        #     range_roi = range_val[y1: y2, x1: x2]
        #     range_roi[np.where(range_roi == 0)] = 9999999
        #     min_range = min(range_roi.reshape(-1))
        #     for i in range(x1, x2):
        #         for j in range(y1, y2):
        #             if range_val[j][i] < min_range * 1.1:
        #                 _, _, z = xyz_destaggered[j][i]
        #                 if z - z_min > z_threshold:
        #                     color_img[j][i] = [0.0, 1.0, 0.0]
...
def main() -> None:
    ...
    default_meta = 'data\lidar\OS2-128_Rev-06.json'
    default_pcap = 'data\lidar\OS2-128_Rev-06.pcap'

    parser = argparse.ArgumentParser(description=descr)
    parser.add_argument('--pause', action='store_true', help='start paused')
    parser.add_argument('--start', type=int, help='skip to frame number', default=0)
    parser.add_argument('--meta', metavar='PATH', help='path to metadata json', default=default_meta)

    parser.add_argument('--sensor', metavar='HOST', help='sensor hostname')
    parser.add_argument('--pcap', metavar='PATH', help='path to pcap file', default=default_pcap)
    ...
    # group.add_argument('--pcap', metavar='PATH', help='path to pcap file', default=default_pcap)
    ...
    try:
        viewer_3d(scans, paused=args.pause, pca=source, meta=metadata)
