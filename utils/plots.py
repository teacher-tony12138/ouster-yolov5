...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        self.lw = line_width if line_width is not None else max(round(sum(im.shape) / 2 * 0.003), 2)
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            ########### Fisher's code ################
            center_point_x = int((p1[0] + p2[0]) / 2)
            center_point_y = int((p1[1] + p2[1]) / 2)
    ...
    def display_distance(self, box, poi, label='', dist=0, color=(128, 128, 128), txt_color=(255, 255, 255)):
        # Add one xyxy box to image with label
        if self.pil or not is_ascii(label):
            self.draw.rectangle(box, width=self.lw, outline=color)  # box
            if label:
            ...
            if dist > 5: 
                cv2.rectangle(self.im, p1, p2, color=(0,255,0), thickness=self.lw, lineType=cv2.LINE_4)
            else:
                cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_4)
